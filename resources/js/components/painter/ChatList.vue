      ...
      <div id="consultation" class="tab-pane show active">
        <template v-if="threads.length > 0">
          <div class="chat-list-item" v-for="(thread, index) in threads" :key="index" @click="click(thread)">
            <div class="data">
              <div class="data-img">
                <img class="card-img-top" :src="thread.user_profile_image" />
              </div>
              <div class="data-body">
                <div class="data-name">
                ...
                <div class="data-content">
                  ...
                  <a v-if="thread.painter_unread_count > 0" href="#">{{ thread.painter_unread_count }}</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <template v-else>
          <div class="chat-list-item">
            <div class="data">
              <div class="data-body">
                <div class="data-name">相談中の利用者は存在しません</div>
              </div>
            </div>
          </div>
        </template>
...
export default {
  ...
  created() {
    this.$store.dispatch('ChatThread/searchPainterThreads', {}).then(() => {
      this.loaded = true;
    });
  },
  methods: {
