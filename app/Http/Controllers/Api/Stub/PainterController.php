...
class PainterController extends Controller
{
    ...
    public function entry(PainterEntryRequest $request)
    {
        ...
        $subject = '会員登録完了通知';
        Mail::to(config('mailsending.contact.send_to'))->send(new Entry($template, $subject, $id, $email, $name));
    ...
    public function show()
    {
        ...
        return response()->json($painter->setAppends(['prefecture_name', 'profile_image', 'image_file1', 'image_file2', 'image_file3', 'image_file4'])->toArray());
    }
    ...
    public function update(PainterEditRequest $request)
    {
        ...
        if (is_array($request->category)) {
            $painter->category = implode(',', $request->category);
        } else {
            $painter->category = $request->category;
        }
...
//        $example->public_consent = $request->public_consent;
    ...
    public function inquiry(Request $request)
    {
        ...
        if ($request->input('confirmed', false)) {
            ...
            $subject = 'お問い合わせ送信通知';
            Mail::to(config('mailsending.contact.send_to'))->send(new Contact($template, $userType, $subject, $email, $name, $contents));
